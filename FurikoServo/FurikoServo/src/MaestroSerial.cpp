...
Maestro::~Maestro()
{
	...
	HANDLE Maestro::openPort(const char * portName, unsigned int baudRate)
{
	HANDLE port;
	DCB commState;
	BOOL success;
...
BOOL Maestro::maestroGetPosition(HANDLE port, unsigned char channel, unsigned short * position)
{
...
* section of the Maestro User's Guide: http://www.pololu.com/docs/0J40 */
BOOL Maestro::maestroSetTarget(HANDLE port, unsigned char channel, unsigned short target)
{
